...
ggplot(data = diamonds) + geom_violin(mapping = aes(x = carat, y = price)) + scale_y_log10() + scale_x_log10()
# Outliers are removed to find average price per carat

# Draw a scatter plot for the diamonds price (y) by carat (x). Color each point
# by the clarity (Remember, this will take a while. Use a sample of the diamonds 
# for faster results)

ggplot(data = diamonds) + geom_violin(mapping = aes(x = carat, y = price, color = "clarity"))

# Change the color of the previous plot using a ColorBrewer scale of your choice. 
# What looks nice?

ggplot(data = diamonds) + geom_violin(mapping = aes(x = carat, y = price, color = "clarity")) + scale_color_brewer(palette = "Spectral")
...
chart <- ggplot(data = diamonds) + geom_bar(mapping = aes(x = cut, fill = cut), width = 1)
chart

# Draw the same chart, but with the coordinate system flipped

flipped_chart <- chart + coord_flip()
flipped_chart

# Draw the same chart, but in a polar coordinate system. It's a Coxcomb chart!

polar_chart <- chart + coord_polar()
polar_chart
...
ggplot(data = diamonds) + geom_point(mapping = aes(x = carat, y = price, color = clarity)) + scale_color_brewer(palette = "Spectral") + facet_wrap(~ color)
